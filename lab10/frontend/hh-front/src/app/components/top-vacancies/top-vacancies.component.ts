...
@Component({
  ...
  template: `
    <div class="mb-8">
      <h1 class="text-3xl font-bold mb-6 text-pink-900">Top 10 Highest Paying Vacancies</h1>
    </div>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr *ngFor="let vacancy of vacancies" class="hover:bg-gray-50">
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="text-sm font-medium text-pink-700">{{ vacancy.name }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <a [routerLink]="['/companies', vacancy.company]" class="text-sm text-pink-600 hover:underline">
                View Company
              </a>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
